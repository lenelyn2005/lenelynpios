...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($username === '' || $password === '') {
        ...
        try {
            $stmt = $mysqli->prepare("SELECT id, password FROM teachers WHERE username = ?");
            $stmt->bind_param("s", $username);
            $stmt->execute();
            $stmt->bind_result($teacher_id, $stored_password);
            $found = $stmt->fetch();
            $stmt->close();
            ...
            if ($found) {
                ...
                if ($isValid) {
                    ...
                    if ($password_info['algo'] === 0) {
                        ...
                        $update = $mysqli->prepare("UPDATE teachers SET password = ? WHERE id = ?");
                        $update->bind_param("si", $newHash, $teacher_id);
                        $update->execute();
                        $update->close();
                    }
...
<head>
    ...
    <title>Teacher Login | College Scheduling</title>
...
<body class="login-page">
    <div class="login-container">
        ...
        <form method="POST">
            ...
            <button type="submit" class="btn btn-secondary" style="width: 100%;"><i class="fas fa-sign-in-alt"></i> Login</button>
        </form>
    </div>
</body>
</html>